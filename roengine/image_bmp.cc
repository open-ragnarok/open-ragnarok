...
ImageBMP::ImageBMP(const std::string& filename, bool flipvertical) : Image(NULL, 0, 0) {
	std::ifstream input;
...
ImageBMP::ImageBMP(const img_t* data, const unsigned int& size, bool flipvertical) : Image(NULL, 0, 0) {
	std::stringstream s;
	s.write((const char*)data, size);
	ReadFromStream(s, flipvertical);
}

bool ImageBMP::ReadHeader(std::istream& input) {
	unsigned int hsize;
...
void ImageBMP::ReadFromStream(std::istream& input, bool flipvertical) {
	using namespace std; 
...
bool ImageBMP::Read8bpp(std::istream& input) {
		// Read 8bpp image
		//glDrawType = GL_BGRA;
		typedef struct {
...
ImageBMP::ImageBMP(const img_t *p, int w, int h) : Image(p, w, h) {
}
