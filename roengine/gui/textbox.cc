...
#include "roengine/gui/textbox.h"
#include "roengine/gui/gui.h"

namespace GUI {

TextBox::TextBox(Element* parent, CacheManager& cache) : Element(parent) {
}

TextBox::TextBox(Element* parent, const TiXmlElement* node, CacheManager& cache) : Element(parent, node, cache) {
	if (node != NULL)
...
void TextBox::Draw(unsigned int delay) {
	float x = 0;
	float y = 0;
...
//				if ((x + gui.calcTextWidth(s.c_str())) > 245) { //200) {
				if ((x + gui.calcTextWidth(s.c_str())) > 200) {
					if (gui.calcTextWidth(s.c_str()) < 200) {
						y += 18.0f;
						x = 10;
					}
				}
