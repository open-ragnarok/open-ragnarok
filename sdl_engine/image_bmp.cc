...
bool ImageBMP::ReadFromStream(std::istream& input, bool flipvertical) {
	...
	switch (infoHeader.bpp) {
		case 4:
			return Read4BPP(input, flipvertical);
			break;
		case 8:
...
bool ImageBMP::Read4BPP(std::istream& input, const bool& flipvertical) {
	int palsize = 16;
	...
	for(int y = 0; y < m_height; y++) {
		for(int x = 0; x < m_width / 2; x++) {
			for (int i = 0; i < 2; i++) {
				if (i % 2)
					idx = (px[(bpr * y) + x] >> 4) & 0x0F;
				else
					idx = px[(bpr * y) + x] & 0x0F;
#if 1
				// This hack is to simulate transparency based on Ragnarok textures using the #FF00FF color for transparent.
				//if ((colors[idx].r >= 254) && (colors[idx].g == 0) && (colors[idx].b >= 254))
				if ((colors[idx].r >= 253) && (colors[idx].g <= 2) && (colors[idx].b >= 253))
					alpha = 0;
				else
					alpha = 255;
#endif
				offset = 4 * (m_width * (flipvertical?(m_height-y-1):y) + x * 2 + i);
...
//	int bytesPerRow = ((m_width * 3 + 3) / 4) * 4 - (m_width * 3 % 4);
	int bytesPerRow = m_width * 3 + (m_width % 4);
	...
	for(int y = 0; y < m_height; y++) {
		for(int x = 0; x < m_width; x++) {
			for(int c = 0; c < 3; c++) {
				this->buffer[4 * (m_width * y + x) + c] =
					pixels[bytesPerRow * (flipvertical?(m_height-y-1):y) + 3 * x + (2 - c)];
			}
			unsigned char r = this->buffer[4 * (m_width * y + x) + 0];
			unsigned char g = this->buffer[4 * (m_width * y + x) + 1];
			unsigned char b = this->buffer[4 * (m_width * y + x) + 2];
			unsigned char alpha;
			if ((r >= 253) && (g <= 2) && (b >= 253))
				alpha = 0;
			else
				alpha = 255;
			this->buffer[4 * (m_width * y + x) + 3] = alpha;
		}
	}
	m_bpp = 32;
#endif
